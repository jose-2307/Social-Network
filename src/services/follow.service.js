...
class followService {
  ...
  async getFriends(userId) {
    ...
    const userFollowings = await Follow.find({
  ...
  async getFriendRecommendations(userId) {
    ...
    const userFollowingsIds = userFollowings.map(
      (following) => following.user2Id
    );
    ...
    for (let x of follows) {
      const friend = await Follow.findOne({
        $or: [
          { user1Id: x._id, user2Id: userId },
          { user1Id: userId, user2Id: x._id },
        ],
      });
      if (!friend) {
